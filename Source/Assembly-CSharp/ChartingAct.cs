...
public class ChartingAct : GameAct
{
	private void Start()
	{
		this.Init();
		if (PlayerPrefs.GetInt("第一次打开制图机") == 0)
		{
			GameController.getInstance().ShowGuideCanvas(13, 14);
			PlayerPrefs.SetInt("第一次打开制图机", 1);
		}
	...
	public void AddCard(CardData cardData, int style = 0)
	{
		...
		Texture2D texture = ImageTools.NormalizeTo256(cardData.StructTexture, this.ClearColors, style);
				...
				if (this.IsBuild)
				{
					foreach (CardData cardData2 in GameController.getInstance().BuildingModMap.Cards)
					{
						if (!(cardData2.StructTexture == null))
						{
							...
							{
								for (int j = 0; j < structTexture.width; j++)
								{
									int num = 0;
					...
					if (!flag)
					{
						...
						CardData cardData3 = CardData.CopyCardData(GameController.getInstance().CardDataModMap.getCardDataByModID("蓝图"), true);
						List<CardData> list = new List<CardData>();
						for (int m = 1; m < this.SourceCardSlots.Count; m++)
						{
							if (this.SourceCardSlots[m].ChildCard != null)
							{
								list.Add(this.SourceCardSlots[m].ChildCard.CardData);
							}
						}
						string value = JsonConvert.SerializeObject(list, Formatting.Indented);
						string value2 = JsonConvert.SerializeObject(cardData, Formatting.Indented);
						cardData3.Attrs.Add("Input", value);
						cardData3.Attrs.Add("Output", value2);
						base.StartCoroutine(base.EmissionEffect(cardData3, this.OutputCardSlots[this.GetEnableOutputCardSlotIndex()], 30));
					}
				}
				else
				{
					foreach (CardData cardData4 in GameController.getInstance().CardDataModMap.Cards)
					{
						if (!(cardData4.StructTexture == null))
						{
							Texture2D structTexture2 = cardData4.StructTexture;
							if (!(texture2D.texelSize != structTexture2.texelSize))
							{
								int n;
								for (n = 0; n < structTexture2.width; n++)
								{
									int num2 = 0;
									while (num2 < structTexture2.height && ((texture2D.GetPixel(n, num2).a <= 0.5f && structTexture2.GetPixel(n, num2).a <= 0.5f) || texture2D.GetPixel(n, num2).Equals(structTexture2.GetPixel(n, num2))))
									{
										num2++;
									}
								...
								if (n == structTexture2.width)
								{
									cardData = CardData.CopyCardData(cardData4, true);
									CardData cardData5 = CardData.CopyCardData(GameController.getInstance().CardDataModMap.getCardDataByModID("蓝图"), true);
									List<CardData> list2 = new List<CardData>();
									for (int num3 = 1; num3 < this.SourceCardSlots.Count; num3++)
									{
										if (this.SourceCardSlots[num3].ChildCard != null)
										{
											list2.Add(this.SourceCardSlots[num3].ChildCard.CardData);
										}
									}
									string value3 = JsonConvert.SerializeObject(list2, Formatting.Indented);
									string value4 = JsonConvert.SerializeObject(cardData, Formatting.Indented);
									cardData5.Attrs.Add("Input", value3);
									cardData5.Attrs.Add("Output", value4);
									base.StartCoroutine(base.EmissionEffect(cardData5, this.OutputCardSlots[this.GetEnableOutputCardSlotIndex()], 30));
									flag = true;
									break;
								}
							}
					...
					if (!flag)
					{
						...
						cardData.Struct = new string[texture2D.width * texture2D.height];
						for (int num4 = 0; num4 < pixels2.Length; num4++)
						{
							for (int num5 = 0; num5 < CommonAttribute.MaterialColors.Length; num5++)
							{
								if (pixels2[num4].Equals(CommonAttribute.MaterialColors[num5]))
								{
									cardData.Struct[num4] = CommonAttribute.MaterialNames[num5];
								}
							}
						}
						cardData.StructWidth = texture2D.width;
						cardData.StructHeight = texture2D.height;
						cardData.StructTexture = texture2D;
						CardData cardData6 = CardData.CopyCardData(GameController.getInstance().CardDataModMap.getCardDataByModID("蓝图"), true);
						List<CardData> list3 = new List<CardData>();
						for (int num6 = 1; num6 < this.SourceCardSlots.Count; num6++)
						{
							if (this.SourceCardSlots[num6].ChildCard != null)
							{
								list3.Add(this.SourceCardSlots[num6].ChildCard.CardData);
							}
						}
						string value5 = JsonConvert.SerializeObject(list3, Formatting.Indented);
						string value6 = JsonConvert.SerializeObject(cardData, Formatting.Indented);
						cardData6.Attrs.Add("Input", value5);
						cardData6.Attrs.Add("Output", value6);
						base.StartCoroutine(base.EmissionEffect(cardData6, this.OutputCardSlots[this.GetEnableOutputCardSlotIndex()], 30));
					}
					this.deltaPos.Clear();
	...
	private IEnumerator ActEndIEnumerator(List<CardSlot> tempList, ChartingAct.ActEndCallBack call)
	{
