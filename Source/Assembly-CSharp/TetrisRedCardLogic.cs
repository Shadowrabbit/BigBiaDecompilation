...
public class TetrisRedCardLogic : CardLogic
{
	public override void Init()
	{
		base.Init();
		this.displayName = LocalizationMgr.Instance.GetLocalizationWord("CT_N_形状斩杀");
		this.Desc = LocalizationMgr.Instance.GetLocalizationWord("CT_D_形状斩杀");
		if (this.CardData != null && GameController.ins != null && GameController.ins.CardDataModMap != null && GameController.ins.CardDataModMap.getCardDataByModID("形状斩杀") == null)
		{
			return;
		}
		if (!this.CardData.HasAttr("TetrisCardLogic_BlockValue"))
		{
	...
	private IEnumerator use()
	{
		...
		for (int j = 0; j < targets.Count; j++)
		{
			if (targets[j])
			{
				ParticlePoolManager.Instance.Spawn("Effect/animal_scratch_1", 0.2f).transform.position = DungeonOperationMgr.Instance.BattleArea[j].CardSlotGameObject.transform.position;
			}
		}
		...
		for (int i = 0; i < targets.Count; i = num + 1)
		{
			...
			{
				yield return DungeonOperationMgr.Instance.DungeonHandler.ChangeHp(DungeonOperationMgr.Instance.BattleArea[i].ChildCardData, -999, this.CardData, false, 0, true, false);
			}
			num = i;
		}
		yield return null;
